...
public class E06Division {
	public static void main(String[] args) {
		...
		if (numb == 0) {
			...
			boolean lSignsDiffer = false; // set true if numa and numb have
											// difference signs
			if (numa < 0)
				lSignsDiffer = (!lSignsDiffer);
			if (numb < 0)
				lSignsDiffer = (!lSignsDiffer);

			// method is to start the "remainder" at numa and subtract numb
			// repeatedly
			...
			if (numa < 0)
				remainder = -remainder;
			System.out.println(numa + " divided by " + numb + " is " + quotient
					+ ", remainder " + remainder);
		}

		// System.out.println("debug to just to check numa/numb= " + numa/numb +
		// " numa%numb= " + numa%numb);
	}
}
